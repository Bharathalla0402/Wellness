//
//  AllProvidersViewController.swift
//  Wellness
//
...
class AllProvidersItemlistCell: UITableViewCell
{
...
class AllProvidersViewController: UIViewController,UITableViewDataSource,UITableViewDelegate,UISearchBarDelegate,CLLocationManagerDelegate,SecondDelegate {
    ...
    var Cell:AllProvidersItemlistCell!
    ...
    var classObject = MultipartViewController()
     ...
     var stritemCategoryId = String()
     var stritemCategoryTypeName = String()
    ...
    {
        let identifier = "AllProvidersItemlistCell"
        Cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? AllProvidersItemlistCell
        if Cell == nil
        {
            tableView.register(UINib(nibName: "AllProvidersItemlistCell", bundle: nil), forCellReuseIdentifier: identifier)
            Cell = tableView.dequeueReusableCell(withIdentifier: identifier) as? AllProvidersItemlistCell
        }
        ...
        Cell.ItemDescription.text = (self.CategorylistArray.object(at: indexPath.row) as! NSDictionary).object(forKey: "description") as? String ?? ""
        let htmlText2 = (self.CategorylistArray.object(at: indexPath.row) as! NSDictionary).object(forKey: "description") as? String ?? ""
                if let htmlData2 = htmlText2.data(using: String.Encoding.unicode) {
                    do {
                        ...
                        Cell.ItemDescription.attributedText = attributedText
                         Cell.ItemDescription.font = UIFont(name: "Helvetica", size: 15)!
                         Cell.ItemDescription.textColor = UIColor.lightGray
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath)
    {
        ...
        self.ListItemsAPIMethod(baseURL: String(format:"%@%@?category_id=%@&lat=%@&long=%@",Constants.mainURL,"product_list",stritemCategoryId,strlat,strlong))
            ...
            DispatchQueue.main.async {
                ...
                if (responceDic.object(forKey: "status") as! NSNumber) == 1
                {
                   ...
                   // myVC?.strSearchType = "2"
                    myVC?.strCategoryId = self.stritemCategoryId
                    myVC?.StrCategoryTypeName = self.stritemCategoryTypeName
                    self.navigationController?.pushViewController(myVC!, animated: true)
                }
                else
                {
...
extension AllProvidersViewController: GMSAutocompleteViewControllerDelegate {
